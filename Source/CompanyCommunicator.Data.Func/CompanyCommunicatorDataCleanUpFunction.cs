...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Data.Func
{
    ...
    public class CompanyCommunicatorDataCleanUpFunction
    {
            ...
            HttpRequestMessage request, ILogger log)
        {
            ...
            {
                return request.CreateResponse(HttpStatusCode.OK, "Error in getting the Row key Id");
            }

            if (string.IsNullOrEmpty(requestPram.SelectedDateRange))
            {
                return request.CreateResponse(HttpStatusCode.OK, "Error in getting the Selected Date Range");
            }

            if (string.IsNullOrEmpty(requestPram.DeletedBy))
            {
                return request.CreateResponse(HttpStatusCode.OK, "Error in getting the Deleted By");
            }

            if (string.IsNullOrEmpty(requestPram.StartDate))
            {
                return request.CreateResponse(HttpStatusCode.OK, "Error in getting the Start Date");
            }

            if (string.IsNullOrEmpty(requestPram.EndDate))
            {
                return request.CreateResponse(HttpStatusCode.OK, "Error in getting the End Date");
            }
